...
def render_predictions_page():
    # 1. Força a renderização do título imediatamente para DESTRAVAR a tela preta
    st.title("🎯 Meus Palpites")
    
    # 2. Importações blindadas dentro da função para capturar quebras no carregamento
    try:
        import datetime
        import pandas as pd
        ...
        st.error(f"💥 Erro Crítico de Carregamento: {import_error}")
        st.info("O problema está acontecendo dentro do database.py ou scoring.py ao serem importados.")
        return

    # 3. Execução do código inteiramente protegida
    try:
        ...
        st.error(f"💥 Erro interno na renderização: {main_error}")
