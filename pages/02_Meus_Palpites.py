...
def render_predictions_page():
    # Título principal fora de qualquer bloco para garantir que a página monte a estrutura básica
    ...
    # Criando as abas de forma isolada
    ...
    with tab_games:
        ...
        try:
            ...
            if not phases:
                ...
                user_preds = db.get_user_predictions(user_id) or []
            ...
            st.error(f"Erro ao carregar a aba de jogos: {e}")
    ...
    with tab_special:
        ...
        try:
            try:
                sp = db.get_special_prediction(user_id) or {}
            except Exception:
                sp = {}
            ...
            if specials_blocked:
                st.warning("🔒 Os palpites especiais foram bloqueados pela administração.")
            ...
            st.error(f"Erro ao carregar os palpites especiais: {e}")
        ...
        try:
            ...
            if not phases:
                ...
                if scoring.can_view_all_predictions(p_status):
                    all_preds = db.get_all_predictions(p_id) or []
                    if not all_preds:
            ...
            all_sp = db.get_all_special_predictions() or []
            if not all_sp:
            ...
            st.error(f"Erro ao carregar auditoria geral: {e}")
